...
def multi_2d(input_tuple):
    ...
    y_finish, y = False, 0
    while not y_finish:
        if y > height - BM3D.PATCH_SIZE:
            y = height - BM3D.PATCH_SIZE - 1
            y_finish = True

        x_finish, x = False, 0
        while not x_finish:
            if x > width - BM3D.PATCH_SIZE:
                x = width - BM3D.PATCH_SIZE - 1
                x_finish = True

            p = Pixel(x, y)
            group_list = BM3D.grouping_2d(slice_2d, p, height, width, used_patches)
            block, block_place = group_list[0], group_list[1]

            block_depth = len(block_place)
            if block_depth > 1:

                block_filtered = BM3D.filtering_2d(block)

                # aggregation
                for block_z in range(block_depth):
                    y_place = block_place[block_z].y
                    x_place = block_place[block_z].x
                    numerator_slice[y_place: y_place + BM3D.PATCH_SIZE, x_place: x_place + BM3D.PATCH_SIZE] \
                        += block_filtered[block_z]

                    denum_slice[y_place: y_place + BM3D.PATCH_SIZE, x_place: x_place + BM3D.PATCH_SIZE] \
                        += 1.0
...
class BM3D(DenoiseMethod):
    ...
    def grouping_2d(data: np.ndarray, patch_p: Pixel, height, width, used_patches: np.ndarray):
        ...
        search_area = Area(patch_p, height, width, BM3D.MATCH_AREA_SIZE, BM3D.PATCH_SIZE)

        find_steps = 0
        while find_steps <= BM3D.MAX_FIND_STEPS:
            find_steps += 1
            ...
            dist = distance2_2d(patch_p, potential_patch, data, BM3D.PATCH_SIZE)
            ...
            if len(block_index) == BM3D.PATCH_SIZE + 1:
                block_index.pop()
                dist_list.pop()
                if find_steps == BM3D.MAX_FIND_STEPS // 1.5:
                    break

        # form 3d array block
        block = np.ndarray((len(block_index), BM3D.PATCH_SIZE, BM3D.PATCH_SIZE), dtype='f4')
        in_p = 0
        for p in block_index:
            block[in_p] = data[p.y: p.y + BM3D.PATCH_SIZE, p.x: p.x + BM3D.PATCH_SIZE]
            in_p += 1
    ...
    def filtering_2d(block: np.ndarray):
        z_block = len(block)
        filtered_block = np.zeros((z_block, BM3D.PATCH_SIZE, BM3D.PATCH_SIZE), dtype='f4')
